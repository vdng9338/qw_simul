...
#ifndef WITHOUT_NUMPY
template <typename Numeric>
PyObject* plot_surface(const std::vector<::std::vector<Numeric>> &x,
...
inline void imshow(void *ptr, const NPY_TYPES type, const int rows, const int columns, const int colors, const std::vector<float> &extent, const std::map<std::string, std::string> &keywords, PyObject** out)
{
    ...
    if(extent.size() != 0) {
        PyObject *ext = detail::get_array(extent);
        PyDict_SetItemString(kwargs, "extent", ext);
    }
...
inline void imshow(const unsigned char *ptr, const int rows, const int columns, const int colors, const std::vector<float> &extent = std::vector<float>(), const std::map<std::string, std::string> &keywords = {}, PyObject** out = nullptr)
{
    detail::imshow((void *) ptr, NPY_UINT8, rows, columns, colors, extent, keywords, out);
}

inline void imshow(const float *ptr, const int rows, const int columns, const int colors, const std::vector<float> &extent = std::vector<float>(), const std::map<std::string, std::string> &keywords = {}, PyObject** out = nullptr)
{
    detail::imshow((void *) ptr, NPY_FLOAT, rows, columns, colors, extent, keywords, out);
}

// NEW
template<typename Numeric> void imshow(const std::vector<std::vector<Numeric>> &data, const std::vector<double> &extent = std::vector<double>(), const std::map<std::string, std::string> &keywords = {}) {
    if(extent.size() != 0 && extent.size() != 4)
        throw std::runtime_error("imshow(): invalid extent");
    if(data.size() == 0)
        throw std::runtime_error("imshow(): no data");

    detail::_interpreter::get();

    // construct args
    //npy_intp dims[3] = { data.size(), data[0].size() };
    PyObject *args = PyTuple_New(1);
    PyObject *arg = detail::get_2darray(data);
...
inline PyObject* figure_size(size_t w, size_t h)
{
    detail::_interpreter::get();
...
inline void close(PyObject *fig) {
    detail::_interpreter::get();
    
    PyObject* args = PyTuple_New(1);
    Py_INCREF(fig);
    PyTuple_SetItem(args, 0, fig);
    ...
    if(!res)
        throw std::runtime_error("Call to close(fig) failed.");
    Py_DECREF(args);
    Py_DECREF(res);
}
...
inline void closeAll() {
    ...
    PyObject* args = PyTuple_New(1);
    PyTuple_SetItem(args, 0, PyString_FromString("all"));
    ...
    if(!res)
        throw std::runtime_error("Call to close('all') failed.");
    Py_DECREF(args);
    Py_DECREF(res);
}
