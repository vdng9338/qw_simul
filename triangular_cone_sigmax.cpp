// Compile command: g++ -Wall -Wextra -std=c++17 -O2 -pthread -I/usr/include/eigen3 -I/usr/include/python3.8 -o triangular_cone_sigmax triangular_cone_sigmax.cpp -lpython3.8
#include <iostream>
#include <cmath>
#include <complex>
#include <Eigen/Dense>
...
void plot(int iGrid = -1) {
    ...
    if(plotCone) {
        ...
        if(maxi == 0.0)
            maxi = 1.0;
        maxi *= 0.6;
        plt::scatter_colored(xlist, ylist, colorlist, 5, {{"cmap","viridis_r"}, {"vmin", "0"}, {"vmax", std::to_string(maxi)}});
    }
    else {
        fig = plt::figure_size(1000,1000);
        plt::set_aspect_equal();
